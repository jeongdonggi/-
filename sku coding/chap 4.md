...
일단 1일 때는 무조건 딸 수 있으므로 따놓는다. 이후 윈도우를 만들어 밀면서 더하고 빼주면 된다.
...
<strong>알고리즘</strong><br>
스택을 사용하여 큰것들부터 넣게하여 내림차순으로 만들어준다. for문을 뒤에서 부터 돌린다. 스택이 비어있지 않고 스택 상단의 값보다 현재 값이 더 크다면 스택 상단에 큰 값인 학생의 번호를 넣어준다.
